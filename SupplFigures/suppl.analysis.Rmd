---
title: 'Supplementary analyses, with plots'
...
data.cor <- data.1 %>%
    select(Pain, bpi_interference, Median_activity_duration)

# Scatterplot 
scatter.plot <- ggplot(data.cor,
                 aes(x = bpi_interference,
                     y = Median_activity_duration,
                     fill = Pain,
                     colour = Pain)) +
    geom_jitter(size = 4, shape = 21, alpha = 0.5) +
    labs(x = '\nMedian pain interference (11-point NRS)',
         y = 'Median duration of activity (minutes)\n',
         title = 'Pain interference vs duration of activity\n') +
...
result.cor <- spearman_test(bpi_interference ~ Median_activity_duration,
                            data = data.cor)
result.cor
```
...
duration.plot <- ggplot(data.duration,
    ...
    geom_jitter() +
    labs(y = 'Median duration of activity (minutes)\n',
...
intensity.plot <- ggplot(data.intensity,
    ...
    geom_boxplot(size = 1, alpha = 0.5) +
    geom_jitter() +
    labs(y = 'Median intensity of activity (counts)\n',
